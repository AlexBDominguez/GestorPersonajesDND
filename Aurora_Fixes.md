...
- **#8.2** — **de los 4 tipos de mecánica de la idea original, solo `RESOURCE_POOL` tiene infraestructura real** (`NUMERIC_BONUS`/`GRANT_PROFICIENCY` siguen hardcodeados como siempre, `GRANT_SPELL` ya era parcialmente declarativo desde antes de #8.2 vía #8.1 — ninguno de los tres se ha tocado en esta sesión). Dentro de `RESOURCE_POOL`: **fases 1/2/3/4/5 hechas**, cubre PHB + las ~88 subclases de Aurora. Commits en `dev`/`origin/dev` de fases 1/2/4: `2c59b23`, `c9a5a59`, `cd2231b` (hechas desde otra máquina el 2026-07-04, no se habían anotado aquí hasta ahora). Fase 3 (`backend/scripts/patch_resource_pool_migration_phase3.sql`), fase 5 (`backend/scripts/patch_resource_pool_migration_phase5_aurora.sql`, + 3 fórmulas nuevas en la DSL) y el fix de `grit` (`patch_fix_grit_recovery_type.sql`) hechos en esta sesión (2026-07-06) y **ya ejecutados por el usuario en el VPS** (falta solo `docker compose up -d --build backend` para que las 3 fórmulas DSL nuevas surtan efecto). Rune Knight: decisión tomada (un recurso por runa), pero es un desarrollo propio de tamaño similar a Way of Shadow/Metamagic (falta contenido de las runas, un selector nuevo en el wizard, y una fórmula DSL nueva) — no implementado, su propio punto de trabajo. Ver detalle completo (incluida la nota de alcance) en el punto #8.2 más abajo.
