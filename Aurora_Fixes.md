...
- **#8.2** — de los 4 tipos de mecánica de la idea original: **`RESOURCE_POOL` ✅ y `GRANT_PROFICIENCY` ✅ tienen infraestructura real (ambos hechos en 2026-07-06); `NUMERIC_BONUS` 🔶 arrancado (infraestructura + 1 caso migrado, Aura of Protection); `GRANT_SPELL` ya era parcialmente declarativo desde antes de #8.2 (vía #8.1) y hoy además cubre los 2 casos PHB que faltaban.** Dentro de `RESOURCE_POOL`: fases 1-5 hechas, cubre PHB + las ~88 subclases de Aurora (fase 3/5 + fix de `grit` **ya ejecutados por el usuario en el VPS** — falta `docker compose up -d --build backend` para las fórmulas DSL nuevas). Dentro de `GRANT_PROFICIENCY`: `RacialTraitService`/`SubclassProficiencyService` reescritos, script `patch_generalize_proficiency_grants.sql` **ya ejecutado por el usuario en el VPS** (incluyó un fix de un bug real: War Domain nunca recibía su bonificador de competencia). Rune Knight: decisión tomada (un recurso por runa) pero es su propio desarrollo, no implementado. Dentro de `NUMERIC_BONUS` (nuevo hoy): extraída una DSL de fórmulas compartida (`CharacterFormulaService`, reutilizada también por `RESOURCE_POOL`), nueva tabla `numeric_bonuses` + campo `grantsBonusKey` en `ClassFeature`/`SubclassFeature`, y Aura of Protection del Paladín migrado como primer caso — corrección importante encontrada: las pruebas de habilidad SÍ se calculan en el backend (se había dicho lo contrario antes en esta sesión), así que ese target también es alcanzable sin tocar Flutter. Fighting Style y los bonificadores de daño (Agonizing Blast, Elemental Affinity...) siguen sin migrar, cada uno con su propio motivo documentado. Script `patch_numeric_bonus_aura_of_protection.sql` sin ejecutar todavía en el VPS. Ver detalle completo en el punto #8.2 más abajo.
...
- **Script:** `backend/scripts/patch_numeric_bonus_aura_of_protection.sql` — siembra el `NumericBonus` y enlaza `class_features.index_name = 'aura-of-protection'` (nombre esperado por convención de dnd5eapi.co, **no verificado contra una auditoría en vivo** — si la `UPDATE` afecta a 0 filas, comprobar el `index_name` real en el VPS).
- **Verificado:** `mvn -o compile`/`mvn -o test` (exit 0). **Pendiente de que el usuario reconstruya el backend y ejecute el script en el VPS**, y confirme con un Paladín real nivel 6+ que las salvaciones siguen sumando el modificador de Carisma correctamente.

**Sin tocar en esta pasada, a propósito:**
- **Fighting Style** (Defense/Archery/Dueling) sigue sin migrar — a diferencia de Aura of Protection, no está atado a una fila de feature con nivel fijo: es una *elección* del jugador (`character.fightingStyle`, resuelta vía `PendingTask`, no vía `resolvedChoiceFor()` genérico como Metamagic — es la única feature que rompe ese patrón). Migrarlo necesita una capa extra: mapear el nombre de la elección resuelta a un `bonusKey` (p.ej. "Defense" → `fighting-style-defense`), que `grantsBonusKey` no cubre tal cual (asume que la fila de feature ya implica el bonus, no que depende de qué opción se escogió).
...
- **Remarkable Athlete** (Campeón: +mitad de la bonificación por competencia, redondeando hacia arriba, a pruebas de STR/DEX/CON en las que no eres competente) — el enganche (`CharacterSkill.getBonus()`) es backend y ya existe, pero la condición ("solo si NO eres competente en esa skill concreta, y solo para 3 características determinadas") es más rica que lo que `condition` soporta hoy (una condición fija por bonus, no una evaluada por-skill en tiempo de cálculo). Necesitaría extender `NumericBonusService`/`CharacterSkill.getBonus()`, no solo sembrar una fila.

**Dónde mirar (pieza `NUMERIC_BONUS`):** `entities/NumericBonus.java`, `entities/ClassFeature.java`/`SubclassFeature.java` (`grantsBonusKey`), `services/CharacterFormulaService.java`, `services/NumericBonusService.java`, `services/PlayerCharacterService.java` (línea de `featureBonusSavingThrows`), `backend/scripts/patch_numeric_bonus_aura_of_protection.sql`, `frontend/lib/views/screens/sheet/tabs/tab_combat.dart` (`_WeaponRow._damageText`), `tab_spells.dart` (`_DamageCell`), `entities/CharacterSkill.java` (`getBonus()`).
