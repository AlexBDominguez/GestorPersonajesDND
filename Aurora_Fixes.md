...
- **Blood Hunter → Mutagen Formula**: la cantidad de fórmulas conocidas depende del modificador de INT y debe gestionarse como un recurso escalable, no fijo. — **Parcialmente hecho.** El wizard (`character_creator_viewmodel.dart:1622-1633`) ya calcula `pickCount: abilityModifier('INT').clamp(1, 99)` para el selector `MUTAGEN_CHOICE` — el escalado por INT en sí ya está resuelto. **Pero la elección nunca se envía ni se persiste en el backend**: no existe ningún endpoint/`PendingTask`/campo de DTO para `MUTAGEN_CHOICE` (confirmado, cero referencias en `backend/src`) — el jugador elige sus fórmulas en el wizard y esa selección se descarta sin guardarse en ningún sitio. Mismo gap ya anotado en el punto #1 ("Gaps conocidos... `MUTAGEN_CHOICE` no tiene ninguna `PendingTask` creada en el backend") y en el punto #8.1 (Blood Hunter, Order of the Mutant). **Este es el único trabajo real que queda de este punto** — decidir cómo persistir "fórmulas de mutágeno conocidas" (¿nueva tabla, o reutilizar el patrón de feat/choice existente?) y conectar el envío del wizard a ello.
