...
- **`PendingTasksScreen` está deshabilitada en el frontend** (`character_sheet_screen.dart`, `// TODO(DASH-02): re-enable PendingTasksScreen once level-up flow is redesigned`, navegación comentada). Las elecciones simples hechas durante el wizard de creación (Fighting Style, Favored Enemy, etc.) se auto-resuelven al crear el personaje via `_autoResolveFeatureChoices`, pero cualquier elección de **seguimiento** generada después de la creación (p. ej. las 3 skills de "Skilled", que solo se crean al resolver el ASI_OR_FEAT) no tiene ninguna pantalla donde mostrarse — quedan como tareas pendientes invisibles para siempre. Esto es probablemente la causa raíz de varios síntomas de "esto no hace nada" que parecen bugs de Aurora pero en realidad son de la mecánica de PendingTasks en general.
