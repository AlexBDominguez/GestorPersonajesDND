...
- **#8 / #8.1** — la auditoría grande de features de clase/subclase sin efecto mecánico. Ya hecho (ver arriba): Channel Divinity, Bardic Inspiration, Ki points, Wild Shape (dedupe), Indomitable, Superiority Dice, Sorcery Points, Mutagen Formula, y ahora (vía #8.2 fase 5) la mayoría de recursos limitados de las subclases de Aurora. Queda: Infusions del Artificiero (el gap más grande, sistema entero inexistente), Rune Knight (diseño pendiente, ver #8.2 fase 5), y el resto de clases/gaps listados en #8.1 (Bárbaro: Totemic Attunement/Frenzy; Bardo: Additional Magical Secrets; Druida: reglas extra de Circle of the Moon; Mago: School features/Empowered Evocation; Pícaro: Fast Hands; Warlock: Pact Magic/Agonizing Blast; Hechicero: Flexible Casting/Metamagic point spend/Draconic Resilience; Blood Hunter: Crimson Rite/Order of the Lycan). Guerrero (Champion) y Sorcerer (Elemental Affinity) ya cerrados vía #8.2 `NUMERIC_BONUS`.
- **#8.2** — de los 4 tipos de mecánica de la idea original: **`RESOURCE_POOL` ✅ y `GRANT_PROFICIENCY` ✅ completos y desplegados en el VPS; `NUMERIC_BONUS` 🔶 en progreso (infraestructura + 3 casos migrados: Aura of Protection y Remarkable Athlete ya desplegados y confirmados, Elemental Affinity pendiente de desplegar); `GRANT_SPELL` ya era parcialmente declarativo desde antes de #8.2 (vía #8.1) y hoy además cubre los 2 casos PHB que faltaban.** Elemental Affinity es el primer caso que toca daño de hechizo (backend: nuevo campo `bonusDamage` en `CharacterSpellSummaryDto`; frontend: `tab_combat.dart`/`tab_spells.dart` ya lo pintan) y el primero cuya condición depende de otra elección del jugador (Draconic Ancestry), no solo "tiene la feature". Rune Knight: decisión tomada (un recurso por runa) pero es su propio desarrollo, no implementado. Quedan dos casos de daño sin migrar, cada uno con su propio motivo (Agonizing Blast: coincide por hechizo concreto + elección multi-select, no un valor único; Empowered Evocation: coincide por escuela + límite de "una vez por turno" que la app no modela) — ver detalle completo en el punto #8.2 más abajo.
...
- **Brujo (Warlock)** — *Pact Magic* (slots de hechizo independientes, se recuperan en descanso corto, no largo) — confirmar que el sistema de slots no los mezcle con los de un multiclase de lanzador (relevante también para el punto #17). *Eldritch Invocations* ya resueltas a nivel de elección (punto #1) pero cada invocation puede tener efecto mecánico propio — **confirmado que Agonizing Blast (+mod. CHA al daño de Eldritch Blast) no aplica nada hoy** (ni siquiera el daño base del cantrip suma modificador de característica, ver #8.2 `NUMERIC_BONUS`); necesario si se juega un Warlock con Eldritch Blast, que es prácticamente todos. *Fiend* (Dark One's Blessing: PG temporales al matar) — trigger basado en evento de combate, normalmente no implementado en apps de ficha.
...
- **Fighting Style** (Defense/Archery/Dueling) sigue sin migrar — a diferencia de Aura of Protection/Remarkable Athlete, no está atado a una fila de feature con nivel fijo: es una *elección* del jugador (`character.fightingStyle`, resuelta vía `PendingTask`, no vía `resolvedChoiceFor()` genérico como Metamagic — es la única feature que rompe ese patrón). Migrarlo necesita una capa extra: mapear el nombre de la elección resuelta a un `bonusKey` (p.ej. "Defense" → `fighting-style-defense`), que `grantsBonusKey` no cubre tal cual (asume que la fila de feature ya implica el bonus, no que depende de qué opción se escogió).
- **Agonizing Blast** (Warlock) — a diferencia de Elemental Affinity, no coincide por *tipo de daño* sino por *hechizo concreto* (solo Eldritch Blast), y la elección de la que depende no es un `resolvedChoiceFor()` de valor único como Draconic Ancestry: Eldritch Invocations es una selección múltiple (varias invocations a la vez, sistema de slots como Battle Master Maneuvers), así que "¿tiene el personaje Agonizing Blast?" es "¿está 'Agonizing Blast' entre las invocations elegidas?", no una sola tarea resuelta. Necesitaría extender el esquema de condición (`MATCH_SPELL_FROM_MULTI_CHOICE:...` o similar) — no es una extensión trivial de lo que ya existe, es un tercer patrón de condición.
- **Empowered Evocation** (Mago, Escuela de Evocación) — coincide por *escuela de hechizo* (Evocation) en vez de tipo de daño, factible con el mismo patrón que Elemental Affinity (`SPELL_SCHOOL_MATCHING`), **pero** el bono real es "una vez por turno", un límite que la app no modela en ningún sitio (no hay concepto de turno/ronda) — aplicarlo sin ese límite sobrestimaría el daño real en combate. Necesita decidir cómo (o si) representar esa limitación antes de implementarlo, no es solo un seed.
...
**Dónde mirar (pieza `NUMERIC_BONUS`):** `entities/NumericBonus.java`, `entities/ClassFeature.java`/`SubclassFeature.java` (`grantsBonusKey`), `services/CharacterFormulaService.java`, `services/NumericBonusService.java` (`bonusFor`, `conditionalSkillBonus`, `spellDamageBonusFor`/`resolveChosenDamageType`), `services/PlayerCharacterService.java` (líneas de `featureBonusSavingThrows`, del bucle de `skillDtos`, y del bucle de `spellDtos`), `dto/CharacterSpellSummaryDto.java` (`bonusDamage`), `backend/scripts/patch_numeric_bonus_aura_of_protection.sql`/`patch_numeric_bonus_remarkable_athlete.sql`/`patch_numeric_bonus_elemental_affinity.sql`, `frontend/lib/models/character/character_spell.dart` (`bonusDamage`), `frontend/lib/views/screens/sheet/tabs/tab_combat.dart` (`_formatSpellDamage`, `_WeaponRow._damageText` — sin migrar, es daño de arma no de hechizo), `tab_spells.dart` (`_formatSpellDamage`, `_DamageCell`), `entities/CharacterSkill.java` (`getBonus()`).
