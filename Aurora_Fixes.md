...
- **#8.2** — **fases 1/2/3/4/5 hechas**, capa de mecánicas de recursos ya cubre PHB + las ~88 subclases de Aurora. Commits en `dev`/`origin/dev` de fases 1/2/4: `2c59b23`, `c9a5a59`, `cd2231b` (hechas desde otra máquina el 2026-07-04, no se habían anotado aquí hasta ahora). Fase 3 (`backend/scripts/patch_resource_pool_migration_phase3.sql`), fase 5 (`backend/scripts/patch_resource_pool_migration_phase5_aurora.sql`, + 3 fórmulas nuevas en la DSL) y el fix de `grit` (`patch_fix_grit_recovery_type.sql`) hechos en esta sesión (2026-07-06). **Pendiente de que el usuario ejecute los tres scripts en el VPS** tras desplegar el backend — ver detalle completo en el punto #8.2 más abajo. Queda sin decidir: Rune Knight (diseño de "pool por runa", ver pregunta al usuario).
