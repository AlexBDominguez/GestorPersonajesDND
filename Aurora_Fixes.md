...
- **#8.2** — de los 4 tipos de mecánica de la idea original: **`RESOURCE_POOL` ✅ y `GRANT_PROFICIENCY` ✅ completos y desplegados en el VPS; `NUMERIC_BONUS` 🔶 en progreso (infraestructura + 2 casos migrados: Aura of Protection ya desplegado y confirmado, Remarkable Athlete pendiente de desplegar); `GRANT_SPELL` ya era parcialmente declarativo desde antes de #8.2 (vía #8.1) y hoy además cubre los 2 casos PHB que faltaban.** Rune Knight: decisión tomada (un recurso por runa) pero es su propio desarrollo, no implementado. El gap más grande que queda dentro de `NUMERIC_BONUS` son los bonificadores de daño (Agonizing Blast, Elemental Affinity, Empowered Evocation) — necesitan trabajo de Flutter, no solo backend, y afectan a clases muy jugadas (Warlock, Hechicero, Mago). Ver detalle completo en el punto #8.2 más abajo.
...
- **Daño de arma/hechizo** (Dueling, Agonizing Blast, Elemental Affinity, Empowered Evocation) — todo frontend, sin centralizar (cada widget calcula el suyo), y el caso de hechizos/cantrips necesita plumbing nuevo desde cero (hoy no hay NINGÚN bonificador de característica aplicado al daño de hechizos, con o sin feature especial). Necesitaría: exponer los `NumericBonus` activos de un personaje por API (mismo patrón que `/api/characters/{id}/resources`), y cablear `tab_combat.dart`/`tab_spells.dart` para leerlos — trabajo de tamaño similar a la fase 4 de `RESOURCE_POOL`, no se ha empezado. Es el gap más grande e importante que queda dentro de `NUMERIC_BONUS` (afecta a Warlock/Hechicero/Mago, clases muy jugadas).
...
**Dónde mirar (pieza `NUMERIC_BONUS`):** `entities/NumericBonus.java`, `entities/ClassFeature.java`/`SubclassFeature.java` (`grantsBonusKey`), `services/CharacterFormulaService.java`, `services/NumericBonusService.java` (`bonusFor`, `conditionalSkillBonus`), `services/PlayerCharacterService.java` (líneas de `featureBonusSavingThrows` y del bucle de `skillDtos`), `backend/scripts/patch_numeric_bonus_aura_of_protection.sql`/`patch_numeric_bonus_remarkable_athlete.sql`, `frontend/lib/views/screens/sheet/tabs/tab_combat.dart` (`_WeaponRow._damageText`), `tab_spells.dart` (`_DamageCell`), `entities/CharacterSkill.java` (`getBonus()`).
