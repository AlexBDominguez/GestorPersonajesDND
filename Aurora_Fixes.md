...
- **#8 / #8.1** — la auditoría grande de features de clase/subclase sin efecto mecánico. Ya hecho (ver arriba): Channel Divinity, Bardic Inspiration, Ki points, Wild Shape (dedupe), Indomitable, Superiority Dice, Sorcery Points, Mutagen Formula, y ahora (vía #8.2 fase 5) la mayoría de recursos limitados de las subclases de Aurora. Queda: Infusions del Artificiero (el gap más grande, sistema entero inexistente), Rune Knight (diseño pendiente, ver #8.2 fase 5), y el resto de clases/gaps listados en #8.1 (Bárbaro: Totemic Attunement/Frenzy; Bardo: Additional Magical Secrets; Druida: reglas extra de Circle of the Moon; Mago: Sculpt Spells/Overchannel del resto de School features; Pícaro: Fast Hands; Warlock: Pact Magic; Hechicero: Flexible Casting/Metamagic point spend; Blood Hunter: Crimson Rite/Order of the Lycan). Guerrero (Champion/Fighting Style), Sorcerer (Elemental Affinity/Draconic Resilience), Wizard (Empowered Evocation) y Warlock (Agonizing Blast) ya cerrados vía #8.2 `NUMERIC_BONUS`.
- **#8.2** — de los 4 tipos de mecánica de la idea original: **`RESOURCE_POOL` ✅, `GRANT_PROFICIENCY` ✅ y `NUMERIC_BONUS` ✅ completos** (los 7 casos identificados en #8.1 que encajaban en el esquema ya migrados: Aura of Protection y Remarkable Athlete ya desplegados y confirmados; Elemental Affinity, Empowered Evocation, Agonizing Blast, Fighting Style y Draconic Resilience pendientes de desplegar); **`GRANT_SPELL` ya era parcialmente declarativo desde antes de #8.2** (vía #8.1) y hoy además cubre los 2 casos PHB que faltaban. **Corrección sobre notas anteriores de esta sesión:** Empowered Evocation no tiene límite de "una vez por turno" (se pensó que sí), y la mitad de CA de Draconic Resilience ya estaba implementada desde antes (solo faltaba la mitad de PG) — ninguno de los dos era el obstáculo que parecía. Rune Knight: decisión tomada (un recurso por runa) pero es su propio desarrollo, no implementado — es lo único que queda pendiente dentro de `NUMERIC_BONUS`/`RESOURCE_POOL`, junto con Dueling/Two-Weapon Fighting (daño de arma, necesita el mismo trabajo de Flutter que ya hizo falta para daño de hechizo). Ver detalle completo en el punto #8.2 más abajo.
...
### 8.2 — pieza `NUMERIC_BONUS` — 🔶 EN PROGRESO (arrancada 2026-07-06)
...
- **Frontend, sin centralizar:** daño de arma (`tab_combat.dart`, `_WeaponRow._damageText` — hoy solo tiene el `+2` de Fighting Style Dueling hardcodeado como un booleano) y daño de hechizo/cantrip (`tab_spells.dart`, `_DamageCell` — **no aplica ningún bonificador de característica hoy, ni siquiera para PHB**; Eldritch Blast muestra su dado base sin nada añadido, así que Agonizing Blast necesitaría construir ese enganche desde cero, no solo generalizar algo que ya exista).
...
**Sexto caso migrado: Fighting Style Defense + Archery — ✅ HECHO (2026-07-07).** El último bonificador backend que quedaba hardcodeado desde el principio de esta pieza (`PlayerCharacterService` comprobaba `"Defense".equalsIgnoreCase(fightingStyle)` directamente). Resuelto reutilizando exactamente el patrón de Agonizing Blast (bono buscado por `targetField`, sin `grantsBonusKey`, condición resuelta contra una `PendingTask`), con dos añadidos:
- Nuevo método `NumericBonusService.fightingStyleBonusFor(character, targetField, wearingArmor)` — a diferencia de Agonizing Blast (que busca un único `bonusKey` conocido), aquí puede haber **varios** `NumericBonus` compitiendo por el mismo `targetField` (a futuro, Dueling/Two-Weapon Fighting podrían añadirse igual), así que escanea todos los `NumericBonus` de ese `targetField` vía el nuevo `NumericBonusRepository.findByTargetField()` y filtra por condición uno a uno.
- `bonus_condition` con formato `"HAS_SINGLE_CHOICE:<taskType>:<valor>"`, con sufijo opcional `;REQUIRES_ARMOR` — Defense lo necesita (el bono solo aplica con armadura equipada, condición que no depende de ninguna elección, es puro estado del personaje) y Archery no.
- `PlayerCharacterService`: el `if/else` sobre `fightingStyle` desaparece, sustituido por dos llamadas (`fightingStyleBonusFor(..., "AC", wearingArmor)` / `fightingStyleBonusFor(..., "RANGED_ATTACK", wearingArmor)`). El campo `fightingStyle` en sí se sigue calculando y exponiendo al frontend igual que antes (lo sigue necesitando para Dueling/Two-Weapon Fighting, que no se han migrado).
- **Script:** `backend/scripts/patch_numeric_bonus_fighting_style.sql` — dos `INSERT`, sin `UPDATE ... grants_bonus_key` (mismo motivo que Agonizing Blast).
- Dueling (+2 daño cma) y Two-Weapon Fighting (daño de característica al offhand) siguen sin migrar — son daño de arma, no de hechizo, viven en `tab_combat.dart` (`_WeaponRow`), mismo tipo de trabajo de Flutter que Agonizing Blast/Elemental Affinity necesitaron para daño de hechizo pero aplicado al daño de arma; no se ha tocado.
...
Con esto, **`NUMERIC_BONUS` cubre los 7 casos identificados en #8.1** que encajaban en el esquema (Aura of Protection, Remarkable Athlete, Elemental Affinity, Empowered Evocation, Agonizing Blast, Fighting Style, Draconic Resilience). Quedan fuera, cada uno con su propio motivo ya documentado: Dueling/Two-Weapon Fighting (daño de arma, no de hechizo, necesita el mismo trabajo de Flutter que el daño de hechizo pero aplicado a `_WeaponRow`), y cualquier cosa "modo de cálculo alternativo" en vez de "bono aditivo" (la mitad de CA de Draconic Resilience ya cubría ese caso desde antes).

**Dónde mirar (pieza `NUMERIC_BONUS`):** `entities/NumericBonus.java`, `entities/ClassFeature.java`/`SubclassFeature.java` (`grantsBonusKey`), `repositories/NumericBonusRepository.java` (`findByTargetField`), `services/CharacterFormulaService.java`, `services/NumericBonusService.java` (`bonusFor`, `conditionalSkillBonus`, `spellDamageBonusFor`/`resolveChosenDamageType`, `spellDamageBonusForSchool`, `spellDamageBonusForNamedSpell`/`hasMultiChoiceValue`, `fightingStyleBonusFor`/`matchesSingleChoice`), `services/PlayerCharacterService.java` (líneas de `featureBonusSavingThrows`, del bucle de `skillDtos`, del bucle de `spellDtos`, de `fightingStyleAcBonus`/`fightingStyleRangedBonus`, del bonus retroactivo tras `applySubclassStatEffects`, y `addHitPoints()`), `services/PendingTaskService.java` (`applyRetroactiveMaxHpBonus`, caso `CHOOSE_SUBCLASS`), `dto/CharacterSpellSummaryDto.java` (`bonusDamage`), `backend/scripts/patch_numeric_bonus_*.sql` (aura_of_protection, remarkable_athlete, elemental_affinity, empowered_evocation, agonizing_blast, fighting_style, draconic_resilience), `frontend/lib/models/character/character_spell.dart` (`bonusDamage`), `frontend/lib/views/screens/sheet/tabs/tab_combat.dart` (`_formatSpellDamage`, `_WeaponRow._damageText` — sin migrar, es daño de arma no de hechizo), `tab_spells.dart` (`_formatSpellDamage`, `_DamageCell`), `entities/CharacterSkill.java` (`getBonus()`).
