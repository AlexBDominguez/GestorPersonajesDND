...
- **#8 / #8.1** — la auditoría grande de features de clase/subclase sin efecto mecánico. Ya hecho (ver arriba): Channel Divinity, Bardic Inspiration, Ki points, Wild Shape (dedupe), Indomitable, Superiority Dice, Sorcery Points, Mutagen Formula. Queda: Infusions del Artificiero (el gap más grande, sistema entero inexistente), y el resto de clases/gaps listados en #8.1 (Bárbaro: Totemic Attunement/Frenzy; Bardo: Additional Magical Secrets; Druida: reglas extra de Circle of the Moon; Mago: School features; Guerrero: Champion/Eldritch Knight/Psi-Warrior sin tracker; Monje: Way of Shadow/Four Elements; Pícaro: Fast Hands; Warlock: Pact Magic/Agonizing Blast; Hechicero: Flexible Casting/Metamagic point spend/Draconic Bloodline; Blood Hunter: Crimson Rite/Order of the Lycan). Además, recordar que este inventario solo cubre las 28 subclases PHB de `seed_subclasses.sql` + Psi-Warrior — el contenido de Aurora (la mayoría de subclases "expandidas") no se ha auditado todavía, ver nota de alcance al final de #8.1. El trabajo más grueso que queda.
- **#8.2** — **parcialmente implementado, no solo diseño** (esto no se había anotado aquí — encontrado el 2026-07-06 revisando `git log`, hecho desde otra máquina el 2026-07-04). Commits en `dev`/`origin/dev`: `2c59b23` (fase 1: DSL de fórmulas + columna `consumesResourceIndexName` en `ClassFeature`/`SubclassFeature`), `c9a5a59` (fase 2: seed SQL de los 11 recursos que antes solo vivían en `_kConsumableFeatures` del frontend — Rage, Ki, Bardic Inspiration, Channel Divinity Clérigo/Paladín, Sorcery Points, Superiority Dice, Action Surge, Indomitable, Second Wind, Lay on Hands, Divine Sense, Arcane Recovery), `cd2231b` (fase 4: `CharacterSheetViewModel` ya lee/escribe estos recursos contra el backend real vía `CharacterClassResourceService`, con fallback automático al mapa efímero antiguo para lo que aún no esté sembrado). **Falta la fase 3** (mencionada en el propio mensaje del commit de fase 4 como pendiente, y no existe ningún commit ni script con ese contenido): enlazar `consumesResourceIndexName` en las 4 features "tiered" de la API pública cuyo `indexName` real no coincide con el recurso sembrado — `bardic-inspiration-d6/d8/d10/d12` → `bardic-inspiration`, `channel-divinity-1-rest/-2-rest/-3-rest` → `channel-divinity-cleric`, `action-surge-1-use/-2-uses` → `action-surge`, `indomitable-1-use/-2-uses/-3-uses` → `indomitable`. Sin esa fase 3, estas 4 siguen cayendo al tracker efímero antiguo (no persisten entre recargas/dispositivos), aunque su recurso ya existe sembrado en BD sin usarse. **Y lo más importante para lo que motivó este trabajo:** la fase 2 solo sembró recursos para clases/subclases "legacy" (PHB) — ninguna mecánica de subclase de Aurora se ha mapeado a esta capa todavía; el diseño (sección debajo) sigue siendo la única referencia de cómo se conectaría el contenido de Aurora, pero no se ha empezado.
...
**Falta — no hay ningún commit ni fichero para esto, a pesar de que el propio mensaje de la fase 4 la da por pendiente:**
- **Fase 3 (enlazar los 4 recursos "tiered"):** Bardic Inspiration, Channel Divinity (Clérigo), Action Surge e Indomitable se desbloquean en la API pública como 3-4 `ClassFeature` independientes por tramo de nivel (p. ej. `bardic-inspiration-d6`/`-d8`/`-d10`/`-d12`, `channel-divinity-1-rest`/`-2-rest`/`-3-rest`), mientras que la fase 2 los sembró como **un solo** recurso colapsado (`bardic-inspiration`, `channel-divinity-cleric`, `action-surge`, `indomitable`). Sin poner `consumesResourceIndexName` en esas filas de `ClassFeature`, `_realResourceFor()` (`character_sheet_viewmodel.dart:669-670`, usa `f.consumesResourceIndexName ?? f.indexName`) nunca encuentra el recurso sembrado para estos 4 y sigue cayendo al tracker efímero de siempre — el recurso existe en BD pero no lo usa nadie. No rompe nada (fallback silencioso), pero la migración se quedó a medias para justo los 4 casos que ya tenían el bug de duplicación de usos (ver #8.1). Pendiente: un script tipo `patch_resource_pool_migration_phase3.sql` con `UPDATE class_features SET consumes_resource_index_name = '...' WHERE index_name IN (...)` para los 4 grupos.
- **Contenido de Aurora sin auditar/mapear a esta capa:** la fase 2 solo cubre recursos de clases PHB ya conocidos ("legacy"). Ninguna subclase o feature llegada vía `AuroraSyncService` se ha revisado todavía para ver si necesita su propia fila `class_resources` — sigue siendo exactamente el gap de la nota de alcance de #8.1 (las subclases de Aurora no están auditadas), ahora con la infraestructura ya lista para recibirlas pero sin ese trabajo empezado.

**Dónde mirar probablemente:** `entities/ClassResource.java`, `entities/CharacterClassResource.java`, `entities/ClassFeature.java`/`SubclassFeature.java` (`consumesResourceIndexName`), `services/CharacterClassResourceService.java`, `frontend/lib/viewmodels/characters/character_sheet_viewmodel.dart` (`_realResourceFor`, `_kConsumableFeatures`), `frontend/lib/services/characters/character_class_resource_service.dart`, `backend/scripts/patch_resource_pool_migration_phase2.sql`, `services/PlayerCharacterService.java` (bonificadores y grants ad-hoc todavía sin migrar), `services/RacialTraitService.java`, `services/SubclassSpellService.java`, `sync/AuroraSyncService.java`.
