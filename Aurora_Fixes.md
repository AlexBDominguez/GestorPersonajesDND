...
- **#8 / #8.1** — la auditoría grande de features de clase/subclase sin efecto mecánico. Ya hecho (ver arriba): Channel Divinity, Bardic Inspiration, Ki points, Wild Shape (dedupe), Indomitable, Superiority Dice, Sorcery Points, Mutagen Formula, y ahora (vía #8.2 fase 5) la mayoría de recursos limitados de las subclases de Aurora. Queda: Infusions del Artificiero (el gap más grande, sistema entero inexistente), Rune Knight (diseño pendiente, ver #8.2 fase 5), y el resto de clases/gaps listados en #8.1 (Bárbaro: Totemic Attunement/Frenzy; Bardo: Additional Magical Secrets; Druida: reglas extra de Circle of the Moon; Mago: Sculpt Spells/Overchannel del resto de School features; Pícaro: Fast Hands; Warlock: Pact Magic; Hechicero: Flexible Casting/Metamagic point spend/Draconic Resilience; Blood Hunter: Crimson Rite/Order of the Lycan). Guerrero (Champion), Sorcerer (Elemental Affinity), Wizard (Empowered Evocation) y Warlock (Agonizing Blast) ya cerrados vía #8.2 `NUMERIC_BONUS`.
- **#8.2** — de los 4 tipos de mecánica de la idea original: **`RESOURCE_POOL` ✅ y `GRANT_PROFICIENCY` ✅ completos y desplegados en el VPS; `NUMERIC_BONUS` 🔶 en progreso (infraestructura + 5 casos migrados: Aura of Protection y Remarkable Athlete ya desplegados y confirmados; Elemental Affinity, Empowered Evocation y Agonizing Blast pendientes de desplegar); `GRANT_SPELL` ya era parcialmente declarativo desde antes de #8.2 (vía #8.1) y hoy además cubre los 2 casos PHB que faltaban.** Los tres últimos casos tocan daño de hechizo (backend: campo `bonusDamage` en `CharacterSpellSummaryDto`, suma de hasta 3 fuentes; frontend: `tab_combat.dart`/`tab_spells.dart` ya lo pintan), cada uno con una condición distinta — Elemental Affinity por tipo de daño elegido (Draconic Ancestry), Empowered Evocation por escuela fija, Agonizing Blast por hechizo concreto dentro de una elección múltiple (Eldritch Invocations) sin fila de feature que enlazar, resuelto buscando el bono directamente por su clave. **Corrección sobre una nota anterior de esta sesión:** Empowered Evocation no tiene límite de "una vez por turno" en las reglas reales (se pensó que sí); no era el obstáculo que parecía. Rune Knight: decisión tomada (un recurso por runa) pero es su propio desarrollo, no implementado. Solo queda Fighting Style sin migrar dentro de `NUMERIC_BONUS` — y ya con diseño claro tras Agonizing Blast, solo falta implementarlo — y Draconic Resilience, que necesita su propio esquema (no es un bono aditivo). Ver detalle completo en el punto #8.2 más abajo.
...
**Sin tocar todavía, a propósito:**
- **Fighting Style** (Defense/Archery/Dueling) sigue sin migrar — aunque el patrón de Agonizing Blast (bonus buscado directamente por `bonusKey`, sin pasar por `grantsBonusKey`) ya demuestra que el problema de "es una elección, no una feature que se tiene" es resoluble, Fighting Style usa un mecanismo de resolución distinto (`character.fightingStyle`, un campo dedicado, no `PendingTask` con `taskType` genérico como `INVOCATION`) — necesitaría su propia condición (`HAS_SINGLE_CHOICE:FIGHTING_STYLE:Defense` o similar) y no se ha hecho todavía, aunque ya no falta diseño, solo implementación.
- **Draconic Resilience** (Hechicero Draconic: +1 PG por nivel + CA alternativa "13 + mod. DEX") no encaja en `NUMERIC_BONUS` tal cual está diseñado — no es "sumar a un campo", es un modo de cálculo alternativo de CA/PG máximos. Necesitaría su propio diseño si se aborda.

**Dónde mirar (pieza `NUMERIC_BONUS`):** `entities/NumericBonus.java`, `entities/ClassFeature.java`/`SubclassFeature.java` (`grantsBonusKey`), `services/CharacterFormulaService.java`, `services/NumericBonusService.java` (`bonusFor`, `conditionalSkillBonus`, `spellDamageBonusFor`/`resolveChosenDamageType`, `spellDamageBonusForSchool`, `spellDamageBonusForNamedSpell`/`hasMultiChoiceValue`), `services/PlayerCharacterService.java` (líneas de `featureBonusSavingThrows`, del bucle de `skillDtos`, y del bucle de `spellDtos`), `dto/CharacterSpellSummaryDto.java` (`bonusDamage`), `backend/scripts/patch_numeric_bonus_*.sql` (aura_of_protection, remarkable_athlete, elemental_affinity, empowered_evocation, agonizing_blast), `frontend/lib/models/character/character_spell.dart` (`bonusDamage`), `frontend/lib/views/screens/sheet/tabs/tab_combat.dart` (`_formatSpellDamage`, `_WeaponRow._damageText` — sin migrar, es daño de arma no de hechizo), `tab_spells.dart` (`_formatSpellDamage`, `_DamageCell`), `entities/CharacterSkill.java` (`getBonus()`).
