...
- **#8.2** — **fases 1/2/3/4/5 hechas**, capa de mecánicas de recursos ya cubre PHB + las ~88 subclases de Aurora. Commits en `dev`/`origin/dev` de fases 1/2/4: `2c59b23`, `c9a5a59`, `cd2231b` (hechas desde otra máquina el 2026-07-04, no se habían anotado aquí hasta ahora). Fase 3 (`backend/scripts/patch_resource_pool_migration_phase3.sql`) y fase 5 (`backend/scripts/patch_resource_pool_migration_phase5_aurora.sql`, + 3 fórmulas nuevas en la DSL) hechas en esta sesión (2026-07-06), sin commit todavía. **Pendiente de que el usuario ejecute ambos scripts en el VPS** tras desplegar el backend — ver detalle completo en el punto #8.2 más abajo. Queda sin decidir: Rune Knight (diseño de "pool por runa"), y un bug preexistente encontrado de paso (recurso `grit` con `recovery_type` incorrecto, no se repone en descanso largo pese a que debería).
...
- **Pendiente de decisión de diseño, no incluido:** Rune Knight (Rune Carver/Master of Runes) — cada runa tiene su propio contador independiente de uso por descanso, duplicado por Master of Runes a nivel 15; sería 6+ recursos casi idénticos o un concepto nuevo de "pool por runa elegida", más una tabla de umbral de nivel nueva. Necesita decidir el enfoque antes de escribir el SQL.
- **Bug preexistente encontrado de paso, fuera de alcance de esta pasada:** el recurso `grit` (Gunslinger) ya sembrado tiene `recovery_type = SHORT_REST`, pero su propio texto dice "Regain all grit on a short **or long** rest" — con la query de recuperación actual, no se repone en descanso largo. No corregido aquí (no es contenido de Aurora), dejar como punto suelto para una futura pasada.
