...
public class CharacterInventoryController {
    ...
    @PostMapping("/{inventoryId}/toggle-identified")
    public ResponseEntity<CharacterInventoryDto> toggleIdentified(
            @PathVariable Long characterId,
            @PathVariable Long inventoryId) {
        
        return ResponseEntity.ok(inventoryService.toggleIdentified(inventoryId));
    }
